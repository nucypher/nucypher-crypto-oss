...
public class WrapperElGamalPRE {

	private ECParameterSpec params;
	private SecureRandom random;

	public WrapperElGamalPRE(ECParameterSpec params, SecureRandom random) {
	...
	public BigInteger rekeygen(PrivateKey sk_a, PrivateKey sk_b){
		BigInteger n = this.params.getN();
		BigInteger inv_sk_b = ((ECPrivateKey) sk_b).getD().modInverse(n);
		BigInteger rk = ((ECPrivateKey) sk_a).getD().multiply(inv_sk_b).mod(n);
		return rk;
	}
	...
	public byte[] encrypt(PublicKey pk, byte[] message) {
		...
		ECPoint[] c = ElGamalPRE.encrypt(params, ((ECPublicKey) pk).getQ(), m, random);
...
//		System.out.printf("x: %s, y: %s\n", c0.getAffineXCoord(), c0.getAffineYCoord());
	...
	public byte[] decrypt(PrivateKey sk, byte[] ciphertext) {
		...
		ECPoint mPoint = ElGamalPRE.decrypt(params, ((ECPrivateKey) sk).getD(), c);
	...
	public byte[] reencrypt(BigInteger rk, byte[] ciphertext) {
		...
		ECPoint[] c_prime = ElGamalPRE.reencrypt(params, rk, c);
...
//		System.out.println("lbits: " + lBits);
		...
		do {
			r = ElGamalPRE.getRandom(sr, ps.getN());
...
//		byte[] p2Bytes = new byte[COMPRESSED_ECPOINT_LENGTH];
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < 10; i++) {
			...
			WrapperElGamalPRE pre = new WrapperElGamalPRE(ecSpec, sr);
