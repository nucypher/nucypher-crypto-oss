 ...
 * @author cygnusv
 */
public class ElGamalPRE {

	public static final String ALGORITHM_NAME = "ElGamal-PRE";
	...
	public static ECPoint[] encrypt(ECParameterSpec ecSpec, ECPoint pk, ECPoint m, SecureRandom sr) {
		...
		ECPoint m_pk_r = pk_r.add(m);
		

		return new ECPoint[] { g_r, m_pk_r };
	}
	...
	public static ECPoint decrypt(ECParameterSpec ecSpec, BigInteger sk, ECPoint[] c) {
		...
		ECPoint pk_r = c[0].multiply(sk).negate();
		ECPoint m = c[1].add(pk_r);
